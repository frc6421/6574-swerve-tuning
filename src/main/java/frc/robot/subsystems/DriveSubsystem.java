...
public class DriveSubsystem extends SubsystemBase {
  ...
  */
  ...
  /**
   * Drive straight using voltage
   * @param volts -12 to 12 volts
   */
  public void setDriveStraightVoltage(double volts){
    for (int i = 0; i < swerveModules.length; i++) {
      swerveModules[i].setDriveStraightVoltage(volts);
    }
  }

  public void setDriveIdleMode(IdleMode mode){
    for (int i = 0; i < swerveModules.length; i++) {
      if (swerveModules[i].setDriveMotorIdleMode(mode) == REVLibError.kError) {
        System.out.println("Module " + i + " IdleMode." + mode.toString() + " not set properly");
      };
    }
  }

  public void setDriveP(double PValue){
    for (int i = 0; i < swerveModules.length; i++) {
      if (swerveModules[i].setDriveMotorP(PValue) == REVLibError.kError) {
        System.out.println("Module " + i + " P not set properly");
      };
    }
  }

  public void setAngleP(double PValue){
    for (int i = 0; i < swerveModules.length; i++) {
      if (swerveModules[i].setAngleMotorP(PValue) == REVLibError.kError) {
        System.out.println("Module " + i + " P not set properly");
      };
    }
  }
  ...
  /**
   * Sets robot velocity for tuning.
   */
  public void setVelocity(double velocity) {
    m_frontLeft.setDesiredState(new SwerveModuleState(velocity, Rotation2d.fromDegrees(0)));
    m_frontRight.setDesiredState(new SwerveModuleState(velocity, Rotation2d.fromDegrees(0)));
    m_rearLeft.setDesiredState(new SwerveModuleState(velocity, Rotation2d.fromDegrees(0)));
    m_rearRight.setDesiredState(new SwerveModuleState(velocity, Rotation2d.fromDegrees(0)));
  }

  public double[] getMaxSwerveModuleVoltage(){
    return new double[] {
            m_frontLeft.getDriveMotorVoltage(),
            m_frontRight.getDriveMotorVoltage(),
            m_rearLeft.getDriveMotorVoltage(),
            m_rearRight.getDriveMotorVoltage()
        };
  }

  public double[] getMaxSwerveModuleOutput(){
    return new double[] {
            m_frontLeft.getDriveMotorOutput(),
            m_frontRight.getDriveMotorOutput(),
            m_rearLeft.getDriveMotorOutput(),
            m_rearRight.getDriveMotorOutput()
        };
  }
